...
Twitter.prototype.stream = function(method, params, callback) {
  ...
  var stream = new streamparser();
    ...
    stream.emit('data', generateTweet());
	}, 250);
...
function generateTweet() {
  ...
  return { created_at: 'Thu Feb 20 01:13:55 +0000 2014',
...
function generateTwitterUser() {
  ...
  return { id: 1111111111,
     ...
     friends_count: 126,
     listed_count: 0,
     created_at: 'Thu Oct 03 20:35:07 +0000 2013',
     favourites_count: 12,
     utc_offset: -21600,
     ...
     profile_image_url: 'http://abs.twimg.com/sticky/default_profile_images/default_profile_5_bigger.png',
     profile_image_url_https: 'https://abs.twimg.com/sticky/default_profile_images/default_profile_5_bigger.png',
...
// Returns randomly generated tweet text
function generateTweetText() {

  var words = ['blah', 'dog', 'cat', 'guy', 'girl', 'blog', 'frog',
                'bat', 'door', 'wall', 'data', 'boop', 'sky', 'arm',
                'foot', 'leg', 'hand', 'fall', 'trip', 'run', 'walk',
                'win', 'jump', 'fly', 'sock', 'hat', 'cap', 'and',
                'not', 'can', 'will', 'make', 'give', 'take', 'car', 
                'that', 'when', 'in', 'from', 'did', 'to', 'or'];
  
  var numWords = parseInt(Math.random() * 24) + 1;
  
  var tweetText = '';
  for(var i = 0; i < numWords;i++) {
    tweetText += (words[parseInt(Math.random()*words.length) - 1] + ' ');
  }
